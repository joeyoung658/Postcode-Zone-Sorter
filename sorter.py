...
def zoneCreater(zones):
    ...
    for i in range(len(zones)):
        ...
        if (found == False):
            counter += 1
            newZones.append([counter, [zones[i][0]], zones[i][1]])
    return newZones
...
def __main__():
    zones = []
    zones = postCodeSorter("monday", zones, monday)
    zones = postCodeSorter("tuesday", zones, tuesday)
    zones = postCodeSorter("wednesday", zones, wednesday)
    zones = postCodeSorter("thursday", zones, thursday)
    zones = postCodeSorter("friday", zones, friday)
    zones = postCodeSorter("saturday", zones, saturday)


    for i in zoneCreater(zones):
        print(i)
